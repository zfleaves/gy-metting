...
async def pull_requirement(request: Request, source_id: str, body: PullRequest):
    ...
    try:
        ...
        for doc_node in doc_nodes:
            try:
                doc_data = client.get_doc(doc_node.slug)
                ...
                content_path = snap_dir / f"{requirement_id}_{doc_node.slug}.md"
                content_path.write_text(f"# {title}\n\n{body}", encoding="utf-8")

                raw_path = snap_dir / f"{requirement_id}_{doc_node.slug}_raw.json"
                ...
                try:
                    snap = Snapshot(
                        source_type="yuque",
                        source_url=f"{source.yuque_url}/{doc_node.slug}",
                        title=title,
